...
the confirm block (400-479) and, after it was confirmed, once on the replicate block (500-579)**; the lock log shows no other slot or block opened (data files are git-ignored; the sha256 of each confirm/replicate output is recorded in its lock file).
...
2. **A small non-FDNA effect was confirmed at tier 2 on a fresh block and re-observed on a second one:** learning the value model on full-information simulation rows and the inference module on cheap
   ...
   R-precision on the fresh confirm block (tier 2: reliable but below the pre-declared 0.05 threshold). **Descriptively, about half of that gap comes from the choice of inference module** (a post-screen observation): decomposition with the MLP inference module gains only +0.016 (P1) / +0.014 (P2) over A8. The replicate run reuses the confirm run's training draws (only test operating points and hidden-state draws are fresh). **Re-observed on the replicate block (500-579): +0.0342 (P1) and +0.0247 (P2), tier 2 again, tier 1 again not met** (rule committed 06:27:36, after the replicate run started at 06:05:50 but before its result existed; training draws are shared with the confirm run, so this is not an independent training replication).
...
Tier 1 (>= 0.05) not supported; tier 2 (reliable, > 0) supported in both cells; tier 3 (negligible) not supported. Descriptive contrasts on the same block (composition minus A8): D_I1_mlp +0.016/+0.014, D_I5_fdna +0.044/+0.029, D_I7_unconstrained +0.041/+0.030, D_I6_shuffled +0.026/+0.008, D_exact +0.015/+0.015 (P1/P2). Replicate block: P1 +0.0342 [+0.0280, +0.0407], P2 +0.0247 [+0.0196, +0.0302] (90% intervals); tier 2 supported, tier 1 and tier 3 not.
