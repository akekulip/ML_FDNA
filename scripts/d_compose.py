...
N_INF = int(os.environ.get("N_INF", 25000)); NJ = int(os.environ.get("N_JOBS", 6)); TOPK = 16
...
if TEST != "test":
    ...
    open_block(TEST, f"D_{VARIANT}_cell{CELL_IDX}")
...
MK = {"D_I1_mlp": lambda: inference.InfMLP(1024), "D_I3_minmax_gnn": lambda: inference.InfGNN(1024),
      "D_I5_fdna": lambda: inference.InfFDNA(lv, "fdna"), "D_I6_shuffled": lambda: inference.InfFDNA(lv, "shuffled"),
      "D_I7_unconstrained": lambda: inference.InfFDNA(lv, "unconstrained")}


def draw(n, seed):
    rng = np.random.default_rng(seed)
    st = w.cidx[v2.sample_states(w, rng, n)] if False else v2.sample_states(w, rng, n)
...
pd.DataFrame(rows).to_parquet(f"{D}/d_{TEST}_{VARIANT}_{CELL_IDX}.parquet")
print("done")
