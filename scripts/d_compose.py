"""Decomposed composition branch (registered before coding): g(op, outage, c) learned on full-information rows, inference
...
def make_mk(rep):
    return {"D_I1_mlp": lambda: inference.InfMLP(1024), "D_I3_minmax_gnn": lambda: inference.InfGNN(1024),
            "D_I5_fdna": lambda: inference.InfFDNA(lv, "fdna"), "D_I6_shuffled": lambda: inference.InfFDNA(lv, "shuffled", seed=rep),
            "D_I7_unconstrained": lambda: inference.InfFDNA(lv, "unconstrained")}
...
for rep in range(REPS):
    ...
    for n in NS:
        ...
        e2e = {"A1_gbm_obs": (lambda d_: np.hstack([d_["Xc"], np.where(d_["obs"] < 0, np.nan, d_["obs"]).astype(np.float32)])),
               "A2_gbm_logic": (lambda d_: np.hstack([d_["Xc"], v2data.logic_features(d_["obs"])])),
               "A8_hybrid_generic_marginals": (lambda d_: np.hstack([d_["Xc"], marg(net1, d_)]))}
        preds = {}
...
out = f"{D}/d_{TEST}_{VARIANT}_{CELL_IDX}.parquet"
