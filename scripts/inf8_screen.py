"""I8 (exact differentiable Bayesian structure, registered before coding) on the SAME draws/scoring as scripts/inf_screen.py. env: VARIANT, CELL_IDX, TEST
...
NS = [int(x) for x in os.environ.get("N_LIST", "1000,5000,25000").split(",")]; REPS = int(os.environ.get("REPS", 3))
...
for N in NS:
    for rep in range(REPS):
        ...
        mk = lambda: BayesStructure(w)
        name = "I8_bayes_structure"
        ...
        print(f"{VARIANT} q={q} s={s} N={N} rep={rep} {time.time()-t0:.0f}s learned fail={np.round(1/(1+np.exp(-net.fail.detach().cpu().numpy())),3)[:4]} stale={float(1/(1+np.exp(-net.stale.item()))):.3f}", flush=True)
out = f"{D}/inf8_{TEST}_{VARIANT}_{CELL_IDX}.parquet"
