"""Inference-only branch (registered before running; re-run 2026-09-24 after the softmin fix). env: VARIANT, CELL_IDX, TEST
...
if TEST != "test":
    slot = f"INF_P{1 if VARIANT == 'v2b' else 2}"
    open_block(TEST, slot, "inf", VARIANT, CELL_IDX)           # lock BEFORE any block data is loaded
w = v2.build_world()
...
for N in NS:
    for rep in range(REPS):
        ...
        MK = {"I1_mlp": lambda: inference.InfMLP(1024), "I3_minmax_gnn": lambda: inference.InfGNN(1024),
              "I5_fdna": lambda: inference.InfFDNA(lv, "fdna"), "I6_shuffled": lambda rep=rep: inference.InfFDNA(lv, "shuffled", seed=rep),
        ...
        # I2: per-generator GBM classifiers on the SAME raw observations, capacity chosen by validation log-loss
        X = lambda o_: np.where(o_ < 0, np.nan, o_).astype(np.float32)
        ova = draw(20000, 7)[0]
        lp = np.zeros((len(Ete), 1024), np.float32)
        for gi in range(5):
            best = None
            for nl in ((7, 15, 31) if TUNE == "screen" else (4, 7, 15, 31, 63)):
                m = lgb.LGBMClassifier(n_estimators=400, num_leaves=nl, learning_rate=0.1, random_state=rep, verbose=-1, n_jobs=NJ)
                m.fit(X(otr), lv[ytr][:, gi], eval_set=[(X(ova), lv[yva][:, gi])], callbacks=[lgb.early_stopping(20, verbose=False)])
                e = m.best_score_["valid_0"]["multi_logloss"]
                if best is None or e < best[0]:
                    best = (e, m)
            m = best[1]
            pg = np.full((len(Ete), 4), 1e-6); pg[:, m.classes_] = m.predict_proba(X(dte["obs"])); pg /= pg.sum(1, keepdims=True)
            lp += np.log(pg)[:, lv[:, gi]]
        evaluate("I2_gbm_product", lp - np.log(np.exp(lp).sum(1, keepdims=True)), N, rep, rows)
        print(f"{VARIANT} q={q} s={s} N={N} rep={rep} {time.time()-t0:.0f}s", flush=True)
out = f"{D}/inf_{TEST}_{VARIANT}_{CELL_IDX}.parquet"
